...
VCHIQ_STATUS_T vchiq_initialise( VCHIQ_INSTANCE_T *instanceOut )
{
   ...
   if (!state)
   {
      printf( "%s: videocore not initialized\n", __func__ );
      goto failed;
   }

   instance = malloc( sizeof(*instance), M_DEVBUF, M_WAITOK);
   if( !instance )
   {
      printf( "%s: error allocating vchiq instance\n", __func__ );
      goto failed;
   }
   ...
   VCHIQ_SERVICE_HANDLE_T       *pservice)
{
   ...
   *pservice = NULL;
   ...
   if ( service  )
   {
      /* XXXBSD: process pid or should be use thread-specific id? */
      status = vchiq_open_service_internal(service, curproc->p_pid);
      if ( status == VCHIQ_SUCCESS )
         *pservice = &service->base;
      else
